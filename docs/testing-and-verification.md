...
| --- | --- | --- |
| `make test-suite` | 13 core geometry suites plus one independent terrain suite, followed by the terrain JSON contract, validation errors, and input-order invariance | `testing/tester.cc`, `testing/tdd_suite.cc`, `testing/suites/*.cc`, `testing/terrain_driver_smoke.sh` |
...
`testing/tdd_suite.cc` aggregates 13 core geometry suites and one independent terrain suite (the aggregate runner is not itself a suite):
